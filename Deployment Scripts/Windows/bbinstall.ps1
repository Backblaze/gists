<#
    BACKBLAZE SILENT INSTALL POWERSHELL SCRIPT
	-groupID <groupID> REQUIRED
	-groupToken <groupToken> REQUIRED
	-userEmail <userEmail> REQUIRED 
	-password <password> REQUIRED
	-sso WILL NOT WORK FOR ACCOUNT SIGN IN, ONLY CREATEACCOUNT
    -adfs
	-csvMapfile <mapfile> 
...
$query = "SELECT * FROM ds_user where ds_sAMAccountName='$currentUser'"
$user = Get-WmiObject -Query $query -Namespace "root\Directory\LDAP"
$user.DS_mail

if ($userEmail -eq "") {

    $userEmail = $user.DS_mail
...
if( -not $ret ) {
    ...
    MyOutput "Temp directory created at [$BACKBLAZE_INSTALL_DIR]"
}
...
if ( -not ( test-path -path "$BBFUPATH" ) ) {
    Invoke-WebRequest -Uri "https://secure.backblaze.com/win32/install_backblaze.exe" -OutFile $BBFUPATH
    MyOutput "Latest Installer Downloaded"
}
...
$installCommand = "./${BACKBLAZE_INSTALLER} -nogui -createaccount $userEmail $password $groupID $groupToken"
$thisDirectory = (Get-Item -force -Path ".\").FullName
...
do {
    ...
    if ( $debug ) { 
        if (testFileLock $BCMD) {
            write-host "[Backblaze installer in use] Locked for $sCount $tUnit"
            if( $password -ne "sso" ) {
                if (Get-Process | Where-Object {$_.MainWindowTitle -eq "bzinst"}) {
                    Get-Process | Where-Object {$_.MainWindowTitle -eq "bzinst"} | stop-process
                    $installCommand = "./${BACKBLAZE_INSTALLER} -nogui -signinaccount $userEmail $password"
                    MyOutput "Email might already have an account? Attemting -signinaccount... about to execute command: [ $installCommand ]"
                    $ierc = Invoke-Expression $installCommand
                }
            }
        } else {
...
if ( $ret -And $ret -ne 1001 ) {
    ...
    MyOutput "Backblaze installed successfully, licensed to $userEmail"
}